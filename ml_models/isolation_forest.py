...
class DetectionIsolationForest:
    def __init__(self):
        ...
        self.freq_encoders = {}  # NEW: dizionario per salvare i mapping di frequency encoding
    ...
    def prepare_data(self, df):
        ...
        print(f"[DEBUG IF] df_final.shape = {df_final.shape}")
        print(f"[DEBUG IF] df_final.columns = {list(df_final.columns)}")
    ...
    def prepare_data(self, df):
        ...
        df["src_port_encoded"] = self.frequency_encode(df, "src_port")
        df["dst_port_encoded"] = self.frequency_encode(df, "dst_port")
        # Fit or transform IP_Flags
        # MODIFICA: usare transform se l'encoder è già fitted, altrimenti fit_transform
        if hasattr(self.one_hot_encoder_ip, "categories_"):
            encoded_ip_flags = self.one_hot_encoder_ip.transform(df[["IP_Flags"]])
        else:
            encoded_ip_flags = self.one_hot_encoder_ip.fit_transform(df[["IP_Flags"]])
        df_encoded_ip_flags = pd.DataFrame(
            encoded_ip_flags,
        ...
        # Fit or transform TCP_Flags
        # MODIFICA: usare transform se l'encoder è già fitted, altrimenti fit_transform
        if hasattr(self.one_hot_encoder_tcp, "categories_"):
            encoded_tcp_flags = self.one_hot_encoder_tcp.transform(df[["TCP_Flags"]])
        else:
            encoded_tcp_flags = self.one_hot_encoder_tcp.fit_transform(df[["TCP_Flags"]])
        df_encoded_tcp_flags = pd.DataFrame(
            encoded_tcp_flags,
        ...
        # MODIFICA: usare transform dell'imputer se è già stato fitted, altrimenti fit_transform
