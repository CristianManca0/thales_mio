...
class RawDetector(base_detector.BaseDetector):
    ...
    def predict_proba(
        ...
        """
        Predict confidence probabilities for samples (Inlier vs Outlier).

        Parameters
        ----------
        X : pd.DataFrame
            The data to compute probabilities for.
        data_path : Path | str | None
            Path to load/save the preprocessed data.
        skip_preprocess : bool, optional
            Whether to skip preprocessing, by default False.
        ...
        -------
        np.ndarray
            Shape (n_samples, 2). The columns correspond to the probabilities
            of the sample being an inlier (class 0) or an outlier (class 1).
        """
        if not self._trained:
            raise ValueError("Model not trained, call fit().")
        ...
        return self._detector.predict_proba(X.values)
