...
class DetectionRandomForest:
    def __init__(self):
        ...
        self.freq_encoders = {}  # ✅ Aggiungi questo

    def frequency_encode(self, df, column_name):
        freq = df[column_name].value_counts()
        encoding = freq.rank(method="dense").astype(int)
        # ✅ Durante il primo fit, salva il mapping
        if column_name not in self.freq_encoders:
            self.freq_encoders[column_name] = encoding.to_dict()
        # ✅ Usa sempre il mapping salvato
        return df[column_name].map(
            lambda x: self.freq_encoders[column_name].get(x, 0)  # 0 per valori mai visti
        )
    ...
    def prepare_data(self, df):
        ...
        # DEBUG: stampa numero colonne
        print(f"[DEBUG RF] df_final.shape = {df_final.shape}")
        print(f"[DEBUG RF] df_final.columns = {list(df_final.columns)}")
    ...
    def prepare_data(self, df):
        df = df.copy()
        # One-Hot Encoding pour les IP_Flags
        if hasattr(self.one_hot_encoder_ip, "categories_"):
            encoded_flags_ip = self.one_hot_encoder_ip.transform(df[["IP_Flags"]])
        else:
            encoded_flags_ip = self.one_hot_encoder_ip.fit_transform(df[["IP_Flags"]])
        df_encoded_flags_ip = pd.DataFrame(
            encoded_flags_ip,
        ...
        # One-Hot Encoding pour les TCP_Flags
        if hasattr(self.one_hot_encoder_tcp, "categories_"):
            encoded_flags_tcp = self.one_hot_encoder_tcp.transform(df[["TCP_Flags"]])
        else:
            encoded_flags_tcp = self.one_hot_encoder_tcp.fit_transform(df[["TCP_Flags"]])
        df_encoded_flags_tcp = pd.DataFrame(
            encoded_flags_tcp,
        ...
        # ✅ FIX: droppa solo le colonne presenti (come isolation_forest)
        columns_to_drop = [
            "IP_Flags",
            "src_ip",
            "dst_ip",
        ...
        # ✅ AGGIUNGI IP_Timestamp solo se esiste (come isolation_forest)
        if "IP_Timestamp" in df.columns:
            columns_to_drop.append("IP_Timestamp")
            # ✅ Aggiungi 'anomaly' solo se esiste (presente solo durante training)
        if "anomaly" in df.columns:
            columns_to_drop.append("anomaly")
        ...
        # MODIFICA: creare imputer come attributo se non esiste
        if not hasattr(self, 'imputer'):
            self.imputer = SimpleImputer(strategy="mean")

        if hasattr(self.imputer, "statistics_"):
            X_imputed = self.imputer.transform(df_final)
        else:
            X_imputed = self.imputer.fit_transform(df_final)
